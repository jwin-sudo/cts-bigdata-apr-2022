object S015_ClassAccessSpecifier extends App{
  ...
  class Order(val id: Int, private val amount: Double, protected var tax: Double){
    println("Order crated ")
    println("ID ", id)
    println("Amount ", amount)
    println("Tax ", tax)
  }

  val order1 = new Order(id = 100, amount = 350, tax = 10)
  println("Order 1 id", order1.id) // works
  //println("Order 1 amount", order1.amount) // error since amount is private to class
  //println("Order 1 tax", order1.tax) // error since tax is protected with in class and derived class
}
