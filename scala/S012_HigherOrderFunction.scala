object S012_HigherOrderFunction extends App{
  ...
  // Double => Double is function f, this function accept double as input, return double as output
  // the function may be square, cube, sqrt tan, cos, ett ..
  // Double * - numbers, variable number of arguments
  // sum function returns double
  // s += f(n), f may be sauare, cube, sqrt, etc anything
  ...
  val sumOfLogoOfNumbers = sum(n => Math.log(n), 5.0, 10.0, 15.0, 20.0)
  println("sumOfLogoOfNumbers", sumOfLogoOfNumbers)
